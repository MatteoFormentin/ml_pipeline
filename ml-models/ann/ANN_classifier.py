...
# Environmental settings (prevents unwanted warnings when modifying databases)
...
# We use two different values because in this problem the Tx power and the Rx power have two different ranges of values,
...
# Collect the columns' names of the input
...
# X = input data described by 35 dimensions (i.e., 35 features)
# y = labels assigned to the input data
# test_size = percentage of the test set
# random_state = seed of the random function, a fixed value is provided to have the same splitting in different
#                executions of the script
# stratify = takes as input the list of the labels, the function provides as output
...
# Parameters explanation:
# n_splits = number of folds in which our training dataset is divided
...
# provide to the object the data that it will split
...
for activation in ['identity', 'logistic']: #, 'tanh', 'relu']:  # Loop over activation functions
    for neurons in [10, 50]: #, 100]:  # Loop over the number of neurons per layer
        for layers in range(1, 2):  #(era range(1, 4)) Loop over the number of hidden layers
            comb += 1
...
performances = open("Performances_result.txt", "w")
